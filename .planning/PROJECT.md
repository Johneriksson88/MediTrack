...
---
*Last updated: 2026-05-24 after Phase 8 (compose-catalog-ux) completion — picker-surface UX sharpening shipped end-to-end (shared AtcCodeCombobox reused across LakemedelFilter + MedicationSheet user-create; differentiated D-140 empty states wired to a new `globalCatalogMatchCount` envelope field; pre-search picker suggestions on `MedicationPickerSheet` driven by a new endpoint + reused dashboard service); 10/10 verifier truths passed, 5 items in `08-HUMAN-UAT.md` await browser smoke testing*
