...
---
*Last updated: 2026-05-25 after Phase 10 (order-numbers) completion — every Order now carries a stable, user-facing `ORD-YYYY-####` identifier minted atomically inside `createDraftOrder`'s `$transaction` via a per-(careUnit, year) `OrderNumberCounter` row-level-locked counter (D-160 — the §6 concurrency answer made concrete), formatted via the shared `formatOrderNumber` utility (D-165 — structured storage + derived display), surfaced at identity-level visual prominence on every order-rendering surface (OrdersTable + DraftsTable leftmost `Best.nr` column, OrdersCardList + DraftCard heading slot, ComposeOrderPage H1, DashboardOrdersCard row primary line, SubmitConfirmationBanner copy); paired DashboardLowStockCard `max-h-[calc(100vh-12rem)]` cap fits the dashboard on screen at all data volumes; 296/296 tests green (api 145 / web 151); 10/10 verifier must-haves verified; 15 advisory code-review findings deferred to post-phase backlog*
