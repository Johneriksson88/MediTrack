...
- [x] Stable, user-facing order numbers (`ORD-YYYY-####`) minted atomically at draft creation. *Validated in Phase 10: order-numbers* (ORD-11; new `Order.orderNumberCounter` + `Order.orderNumberYear` columns (both NOT NULL, composite `@@unique([careUnitId, orderNumberYear, orderNumberCounter])`) + new `OrderNumberCounter` per-(careUnit, year) counter table; `mintOrderNumber()` runs **inside `createDraftOrder`'s `$transaction`** using a two-step UPDATE-then-INSERT-with-ON-CONFLICT pattern (D-160) — row-level write-lock primitive serializes concurrent mints, atomic with `order.create`, deterministic per-(careUnit, year) sequence (the §6 "two nurses ordering simultaneously" concurrency story made concrete); shared `formatOrderNumber({year, counter})` utility is the single source of truth for the `ORD-YYYY-####` display (D-165 — structured storage + derived display so format changes never require migration); `orderResponse` / `orderListItem` / `dashboardOrderRow` Zod contracts widened with `orderNumber` as REQUIRED string; migration 0013 backfills every existing Order row deterministically (`ROW_NUMBER` over `(careUnit, year)` ordered by `createdAt ASC`) and seeds the counter table from `MAX + 1`; audit allowlist extended with both immutable persisted counter columns; FE surfacing: leftmost `Best.nr` column on OrdersTable + DraftsTable (`font-mono`, identity-on-the-left), OrdersCardList + DraftCard top-of-card heading slot promotes orderNumber (D-167 — H1 carries identity, OrderStatusPill carries lifecycle), ComposeOrderPage H1 reads `Beställning ORD-YYYY-####`, DashboardOrdersCard row primary line is orderNumber (`font-mono`), SubmitConfirmationBanner copy `Beställning ORD-YYYY-#### är skickad.`; sc04 mobile screenshot harness re-run with all 24 cells PASSED + harness fix to resolve `/bestallningar/ny` via `GET /api/orders?status=utkast` (would 404 otherwise); paired follow-up cap on `DashboardLowStockCard` (`max-h-[calc(100vh-12rem)]`) constrains the 3433-row low-stock list to viewport-minus-chrome; 296/296 tests green (api 145 / web 151); 10/10 verifier must-haves verified; 15 advisory code-review findings deferred — top 3: CR-02 TZ year-drift pin in `EXTRACT(YEAR …)`, CR-04 extract `mintOrderNumber` helper to deduplicate seed.ts, CR-09 aria-label temporal cue a11y regression)
...
- [x] Logout reachable at every breakpoint + Konto admin-gate copy verbatim. *Validated in Phase 11: quick-polish* (UX-02, UX-03; TopBar exposes two sibling Logga ut buttons — mobile `md:hidden h-11 w-11` icon-only with `aria-label="Logga ut"` + desktop `hidden md:flex min-h-[44px]` icon+label — both wiring through the existing single-source-of-truth `useLogout()` hook (D-170/D-174); UserPillPopover collapsed to a static non-interactive UserPill identity strip via `git mv` rewrite per D-171/D-175; KontoPage.tsx:115 gate note swapped verbatim to "Ändringar kan endast göras av administratör." with trailing period preserved per ROADMAP SC#2 (D-173); destructive Konto logout button retained verbatim per D-172; 164/164 web tests green incl. 6 new TopBar.test.tsx tests; sc04 Playwright harness exited 0 (24/24 cells) — 5 sc04-360-*.png screenshots regenerated showing the new mobile logout icon; pure FE diff — no new endpoints, no schema, no env vars)
...
---
*Last updated: 2026-05-26 after Phase 11 (quick-polish) completion — closes UX-02 + UX-03, the final two v1 requirements. TopBar exposes Logga ut at every breakpoint via per-breakpoint sibling buttons (mobile `md:hidden` 44×44 icon-only + desktop `hidden md:flex` icon+label) both wired through the existing single-source-of-truth `useLogout()` hook (D-170/D-174); the desktop UserPillPopover is collapsed to a static non-interactive UserPill identity strip via `git mv` rewrite (D-171/D-175); KontoPage.tsx:115 gate-note for sjuksköterska/apotekare reads exactly "Ändringar kan endast göras av administratör." with trailing period preserved per ROADMAP §Phase 11 SC#2 canonical quote (D-173); the existing destructive Konto logout button is retained verbatim per D-172. Pure FE diff — no new endpoints, no schema, no env vars, no new dependencies. 164/164 web tests green including 6 new TopBar.test.tsx tests; sc04 Playwright harness exited 0 (24/24 cells) with 5 sc04-360-*.png screenshots regenerated. 8/8 verifier must-haves passed; code review 1 advisory Warning (KontoPage doc/code mismatch pre-existing — not phase-11 code) + 4 Info findings deferred. v1 milestone scope complete.*
