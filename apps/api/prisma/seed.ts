 ...
 *
 * Idempotent: checks for an existing utkast Order for the same
 * (careUnitId, createdByUserId) pair before inserting to prevent duplicate
 * drafts across reruns (per D-62 plan seed spec).
 *
 * Lines prefer CareUnitMedications where currentStock < lowStockThreshold so
 * the demo path includes the LowStockBadge in the picker scenario (D-61).
 */
async function seedDraftOrder(prisma: PrismaClient): Promise<void> {
  ...
  if (!sjukskoterska) {
    // eslint-disable-next-line no-console
    console.log('[seed] sjukskoterska user not found — skipping draft order seed.');
    return;
  }

  // Idempotency check: skip if an utkast draft already exists for this nurse+unit.
  ...
  // Pick 3 low-stock CareUnitMedications for the demo path (D-61).
  // Prisma doesn't support column-to-column comparisons in where clauses,
  // so we fetch a small batch and filter in JS.
  const candidateMeds = await prisma.careUnitMedication.findMany({
    where: {
      careUnitId: sjukskoterska.careUnitId,
      deletedAt: null,
    },
  ...
  const lowStockMeds = candidateMeds
    .filter((m) => m.currentStock < m.lowStockThreshold)
    .slice(0, 3);

  // Fallback: if fewer than 3 low-stock meds, pad with any available meds.
  let medIds = lowStockMeds.map((m) => m.id);
  if (medIds.length < 3) {
    const fallback = await prisma.careUnitMedication.findMany({
      ...
      take: 3 - medIds.length,
      orderBy: { createdAt: 'asc' },
      select: { id: true },
    });
    medIds = [...medIds, ...fallback.map((m) => m.id)];
  }

  if (medIds.length === 0) {
    // eslint-disable-next-line no-console
    console.log('[seed] No CareUnitMedications available — skipping draft order seed.');
    return;
  }
  ...
  await prisma.order.create({
    data: {
      ...
      status: 'utkast',
      lines: {
        create: medIds.map((careUnitMedicationId) => ({
